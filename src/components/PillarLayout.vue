...
const categoryOptions = computed(() => {
  ...
  return rawCategories.map(normalizeCategoryOption).filter((category) => {
...
const tagOptions = computed(() => {
  ...
  return rawTags.map(normalizeTagOption).filter((tag) => {
    if (!tag) return false
...
const getItemCategory = (item) => {
  ...
  if (
    ...
    typeof item.category.value === 'string'
  ) {
...
const normalizeTagValue = (value) =>
  value?.toString().replace(/^#/, '').trim().toLowerCase() || null
...
const filteredFeed = computed(() =>
  feedItems.value.filter(
    (item) => itemMatchesSearch(item) && itemMatchesCategory(item) && itemMatchesTags(item),
  ),
)

const sortFilters = computed(
  () => props.config.sortFilters ?? ['Latest', 'Most Popular', 'Reward Points'],
)
...
<template>
  <section class="flex flex-col gap-3" :style="accentStyles">
    <!-- Banner -->
    <header
      class="grid gap-6 sm:grid-cols-2 rounded-3xl bg-white dark:bg-slate-900/80 p-8 shadow-banner ring-1"
    ...
    >
      ...
      <div class="grid gap-3">
        <div class="flex items-center gap-4">
          ...
          <h1 class="text-3xl font-bold text-slate-900 dark:text-slate-100 md:text-[40px]">
            {{ headline }}
          </h1>
        </div>

        <p v-if="config.description" class="max-w-3xl text-base text-slate-600 dark:text-slate-300">
          {{ config.description }}
        </p>
      </div>
      ...
      <!-- Highlights -->
      <div
        v-if="config.highlights?.length"
        class="flex flex-col items-end gap-3 self-start justify-self-end text-sm text-slate-600 dark:text-slate-300"
      >
        <span
    ...
    <div
      :class="[
        ...
        sidebarExpanded ? 'lg:grid-cols-[minmax(0,_260px)_1fr]' : 'lg:grid-cols-[max-content_1fr]',
      ]"
    >
      <PillarSidebar
      ...
      <div class="grid gap-3">
        <!-- Create post prompt -->
        <div class="rounded-4xl p-2" :style="{ backgroundColor: accentWithAlpha('20') }">
          <button
            type="button"
            class="flex w-full items-stretch rounded-3xl bg-white dark:bg-slate-800/60 shadow-panel ring-1"
            :style="{ '--tw-ring-color': 'var(--pillar-ring-strong)' }"
            @click="openForm"
          >
            <div class="flex flex-1 items-center gap-4 px-6 py-3">
              <span
                class="grid h-12 w-12 place-items-center rounded-full bg-indigo-50 dark:bg-indigo-900/30 text-2xl"
                :style="{ color: accentColor }"
              >
                ✍️
              </span>
              <span class="flex-1 text-base font-medium text-slate-500 dark:text-slate-300">
          ...
          <slot name="feed" :items="filteredFeed" :accent="accentColor" :filters="activeFilters">
            ...
            >
              <div
                ...
                class="flex flex-col items-center justify-center gap-3 rounded-3xl border border-dashed border-slate-300 dark:border-slate-700 bg-white/60 dark:bg-slate-800/50 p-10 text-center text-slate-500 dark:text-slate-300"
              >
                <span class="text-4xl">🛎️</span>

                <p class="text-base font-semibold">No posts match the current filters</p>
                <p class="text-sm text-slate-500 dark:text-slate-300">
                  Try adjusting your search or be the first to create a post and earn Stunix from
                  the community.
                </p>
    ...
    >
      <form
        class="w-full max-w-xl rounded-2xl bg-white dark:bg-slate-900/90 p-8 shadow-2xl ring-1"
          ...
          <slot name="form-fields" :form-state="formState" :config="config">
            <label class="grid gap-2 text-sm font-semibold text-slate-800 dark:text-slate-200">
              ...
              <input
                ...
                class="w-full rounded-2xl border border-slate-300/60 dark:border-slate-700/50 dark:bg-slate-800/60 dark:text-slate-100 px-4 py-3 text-base outline-none transition focus:border-indigo-300 focus:ring-2 focus:ring-indigo-200"
              />
            </label>
            ...
            <label class="grid gap-2 text-sm font-semibold text-slate-800 dark:text-slate-200">
              ...
              <input
                ...
                min="5"
                step="5"
                class="w-32 rounded-2xl border border-slate-300/60 dark:border-slate-700/50 dark:bg-slate-800/60 dark:text-slate-100 px-4 py-3 text-base outline-none transition focus:border-indigo-300 focus:ring-2 focus:ring-indigo-200"
              />
            </label>
          </slot>
        </div>
...
<style scoped>
:deep(.shadow-panel) {
  --tw-shadow: var(--pillar-shadow-banner, 0 30px 70px -30px rgba(67, 56, 202, 0.18));
  --tw-shadow-colored: var(--pillar-shadow-banner, 0 30px 70px -30px rgba(67, 56, 202, 0.18));
  box-shadow:
...
.shadow-panel {
  ...
  --tw-shadow-colored: var(--pillar-shadow-panel, 0 12px 32px -20px rgba(79, 70, 229, 0.35));
  box-shadow:
    var(--tw-ring-offset-shadow, 0 0 #0000), var(--tw-ring-shadow, 0 0 #0000), var(--tw-shadow);
}
</style>

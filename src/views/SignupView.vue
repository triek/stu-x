...
const loginLink = computed(() => buildRedirectRoute('login', redirectPath.value))
...
<template>
  <section
    class="mx-auto grid max-w-6xl gap-12 rounded-3xl bg-white/90 dark:bg-slate-900/90 p-8 shadow-primary ring-1 ring-indigo-100/60 lg:grid-cols-[1.1fr_0.9fr] lg:p-14"
  >
    <div class="space-y-6">
      <div class="space-y-2">
      ...
      >
        ...
        <ul class="mt-4 grid gap-3 text-sm text-slate-600 dark:text-slate-300">
          <li class="flex items-start gap-3">
            ...
            ><span>Personalized insight prompts and survey opportunities.</span>
          </li>
          <li class="flex items-start gap-3">
            <span class="mt-1 text-brand">◆</span
            ><span>Earn verified Stunix for your time and expertise.</span>
          </li>
          <li class="flex items-start gap-3">
            <span class="mt-1 text-brand">◆</span
            ><span>Exclusive access to StuX community cohorts and events.</span>
          </li>
        </ul>
      </div>
      <p class="text-sm text-slate-500 dark:text-slate-300">
        Already have an account?
        <RouterLink :to="loginLink" class="font-semibold text-brand hover:underline"
    ...
    <form
      class="grid gap-6 rounded-3xl border border-indigo-100/70 bg-white/85 dark:bg-slate-900/80 p-6 shadow-panel"
      @submit.prevent="handleSubmit"
    >
      <div class="grid gap-2">
        <label for="username" class="text-sm font-semibold text-slate-700 dark:text-slate-200">
          Username
      ...
      <div class="grid gap-2">
        <label for="school" class="text-sm font-semibold text-slate-700 dark:text-slate-200"
          >School or organization</label
        >
        <input
          id="school"
          type="text"
      ...
      <div class="grid gap-2">
        <label for="region" class="text-sm font-semibold text-slate-700 dark:text-slate-200">
          Country or region
          <span class="text-rose-500" aria-hidden="true">*</span>
          <span class="sr-only">required</span>
        </label>
      ...
      <label class="flex items-start gap-3 text-xs text-slate-500 dark:text-slate-300">
        <input
          type="checkbox"
          name="terms"
          class="mt-1 h-4 w-4 rounded border-indigo-200 dark:border-slate-700 text-brand focus:ring-indigo-200 dark:bg-slate-800"
          v-model="formState.termsAccepted"
          required
          aria-required="true"
        />
        <span>
          I agree to the StuX community charter and allow StuX to contact me with onboarding
          resources
          <span class="text-rose-500" aria-hidden="true">*</span>
          <span class="sr-only">(required)</span>
        </span>
      ...
      <button type="submit" class="btn btn-brand" :disabled="isSubmitDisabled">
        <span v-if="isSubmitting" class="flex items-center gap-2">
          <span class="h-2.5 w-2.5 animate-ping rounded-full bg-white dark:bg-slate-700"></span>
          Creating account...
        </span>
        <span v-else>Sign up</span>
      </button>
      <p class="text-xs text-slate-400 dark:text-slate-500">
        We protect your data and only use it to personalize your StuX experience.
      </p>
    </form>
  </section>
</template>
